...
- **Implementation process:** coordinator runbook `.claude/skills/p3-implementation/SKILL.md` + **orientation snapshot `P3/HANDOFF.md`** (read first — where the build stands, the next act, the open gate batch, the invariants) + live state `P3/STATE.md` (**the single source of truth; outranks the handoff**). Any session continues the build via "continue implementation" (or /p3-implementation); work runs in spec-referenced packets executed by fresh-context subagents, with operator gates at each B-phase boundary.
