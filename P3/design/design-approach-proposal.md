...
## 7. Post-approval amendments (dated, per §5's own mechanism)

**A1 — 2026-08-05 (UI-7 grounding conflict findings; coordinator-authored at the OQ1/OQ5/OQ6 dispositions, recorded in `P3/STATE.md` this date).** Three §3 sentences narrowed to what the frozen wire and the binding rules allow; the ratified scope is otherwise unchanged.

1. **The degraded-mode example sentence.** §3's illustration — *"no local model is wired, so I can't read sentences yet — pick a query"* — cannot ship as unconditional client copy: the disambiguation card carries no machine cause code on the wire (`{question, reason, choices[]}`), the store serves cards for at least seven distinct reason classes, and §38 bans classifying served prose. The shipped shape: general operator words about what the card IS and what picking a choice does, with the platform's own served `reason` rendered verbatim as the sole statement of cause. A served cause code, if ever wanted, is its own sanctioned backend decision post-batch.
2. **"One-shot first-use hints"** narrows to once per tab session, held in memory: §41-B bans web storage, and no server-side per-person UI-state store exists (building one is beyond the batch freeze). Hints re-appear on the next full load; the tour never auto-starts for the same reason.
3. **"Completion toasts"** at v0 means the chat turn lifecycle (`chat.turn_settled` / `chat.turn_abandoned` arriving while the person is elsewhere): run and deliverable frames carry no top-level state fact and their payloads are opaque to the shell under the frames-are-triggers discipline, so a run-grain "finished" toast would fabricate a fact the frame does not state. Run-grain completion toasts are a post-batch candidate needing a served state fact (its own sanction).
