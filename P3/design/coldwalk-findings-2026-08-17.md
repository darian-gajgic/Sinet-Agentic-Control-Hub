...
## Return visit (alice, her own world, post-RA-B1/RW-17 build): **ALL 5 ERRANDS PASSED — no block.** Website viewed rendered + judged, downloaded (9,301 B on disk), checker's findings read ("caught yesterday's exact disaster in writing"), ACCEPT completed with PIN at $0 ("State: accepted · no commit · no effect"), ACCEPTED chip + decision row confirmed. **7 frictions → the final builder polish round:** (1) the preview letterbox scroll trap — lower half of her own site unviewable rendered (wheel scrolls outer page, keyboard dead, hard-eject past section 2); (2) the page's intro boilerplate contradicts the pinned arm ("a commit pushed under your own credentials" vs the card's "nothing is pushed") — suppress the repo prose on pinned deliverables; (3) "Launch a preview" enabled-but-dead answering in spec citations — disabled-with-honest-reason or hidden when no-preview is the defined state; (4) the final verdict unreadable (dud "verdict #N — findings below" link; run.summary_written with no reader); (5) the acceptance not in waiting-on-you while 13 alien cards are; (6) spec-language leaks (ongoing G-class); (7) oddities: completed check's elapsed timer still ticking, "opened 3h ago" for yesterday, alt-text describes a couple over a strawberry placeholder, the download is a fenced .md not a browser-openable page.

## Re-walk B (spouse, kept world): **ERRAND 1 FAILS ON ITS CORE — BLOCKS.** Found the cancelled task in seconds (search "cancel" → the CANCELLED — OPEN FOR WHY badge: the board fixes WORK); but the WHO/WHY is unreachable on every surface, and the card itself admits it: "the cancel is recorded on the run's transition record, which is not yet served into this list — a platform gap, reported." Errand 2 completed (verdict SHIP-with-notes trusted "narrowly — because I could check it myself"; cross-family judge visible); errand 3 completed (USD 0 priced, honestly unpriced). Phone 616px holds structurally.

**→ P3-RW-18 (backend batch, all previously G-listed, now walk-proven):** D1 the cancel's actor+reason served into the task's HUMAN DECISIONS (the transition record exists; the serve is missing); D2 deliverable mint carries `project_id` (the empty mint now wrongly CLOSES accept on a repo-backed task — "belongs to no project" on parts-catalog's own deliverable — and forced the earlier view-side bucketing workaround); D3 verify's duplicate identical finding (same text+timestamp filed twice) deduped at mint; D4 History own-words ask: a confidence floor (wrong-match margin 8.575 answered confidently; a miss must say "no view matches — here's what I have") + a `tasks.cancelled` canned view (fifty canned questions, none contains "cancelled"). **→ Final builder polish round:** alice's 7 + walk-B view items (dud verdict link, black void at page foot, Fleet's stacked Pause+Resume both offered, drawer auto-close on phone nav, DONE-vs-in-review wording, the old-era plan card's assumption walls on TaskDetail's render path, pre-sign-in dashboard exposure, cost-column cutoff at narrow).
