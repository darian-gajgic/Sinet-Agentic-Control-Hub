...
2. **`sandbox-runtime` — RECLASSIFIED 2026-07-20 from "deviation to ratify" to "spec-conformance defect to remediate"** (operator challenge; coordinator re-read S11.1). **S11.1 "Adoption" makes srt a BINDING adoption** ("Anthropic's Apache-2.0 `sandbox-runtime` is adopted for the bwrap+seccomp+proxy core [G2 D2.2]"); direct composition is the S16.3 **funeral plan** (execute on abandonment), NOT an authorized alternative. B1-3 silently shipped the funeral-plan fallback instead of adopting — violating adopt-don't-fork's gate-ratification rule + the "never resolve a spec conflict silently, stop-and-amend" rule. It also cost coverage (srt's Node-safe positive seccomp allowlist → replaced by a weaker default-allow filter; standard proxy egress shape → deferred). Coordinator under-caught it in B1-3 validation. **My earlier "ratify" recommendation is WITHDRAWN.** This is NOT a bless-it item. Two clean paths: **(A)** restore spec fidelity — re-cut B1-3's composition to actually adopt srt (invoke it as the external process-wrapper per S11.1; direct-composition demoted to funeral plan in components.lock); **(B)** deliberate S00.9 amendment making direct-composition primary + srt standby/removed, operator-approved with T09-review. Choice hinges on srt's live mid-2026 status (real-world fact → must live-verify). Contained + cheap to fix now: dev-mode only, no host install shipped, srt-shaped host pieces already deferred seams. Recommendation: **live-verify srt status, then A if healthy / B if a real blocker exists** — awaiting operator direction.
