...
**Status: CLOSED 2026-07-19 — all five decisions answered by the operator (step-by-step walkthrough, free-text answers); B1 opened.** Outstanding hands-on: operator item 3 (probe batch) rides early B1; steps in `P3/measurements/PROBES.md`. Written 2026-07-19 at B0 queue completion (5/5 packets done, coordinator-validated, CI green on every push). Contract: `Spec/core-architecture-v1.md` v1 (tag `spec-v1`) + amendment A1. Build record: `P3/STATE.md` log; per-packet readings in the packet commit bodies.
...
- **2026-07-19 — Decision 3: option (i) — ALL host-side installs deferred to the B2 gate** (operator "ok" on the recommendation). No units installed, no cert issued, no Caddy at B0/B1; B1 runs dev-mode only. At B2: install control(+broker) units + journald drop-in, run `B0-cert-steps.md` (CT publication + Caddy onboarding + lock entry), batched as one reviewed host-change moment. Caveats logged: broker/portpool modes are stubs until B1; a pre-release-pipeline install is a hand-copied binary (dev-phase deviation from S01.11, to be noted at B2 if the release packet hasn't landed).
- **2026-07-19 — Decision 4: option (c) — logind PrepareForSleep/inhibitor wiring deferred to B2**, decided alongside the service install it exists to protect; the adopt-a-D-Bus-module vs hand-implement choice is made at B2 with live-verified library facts. Interim posture stays: suspend = crash-equivalent, wake-grace + recovery ladder carry correctness; the gap costs only re-spend bounds on a not-yet-installed service.
- **2026-07-19 — Decision 5: RATIFIED EN BLOC** — all 40 remaining clearly-implied readings across B0-2..B0-5 (section-cited in the packet commit bodies; headline set walked through in-session). They are settled build interpretations, not spec amendments; supersession paths (e.g. S14 event-naming at B5) stand as planned. **GATE CLOSED.**
