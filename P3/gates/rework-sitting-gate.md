...
### B13. Rubric v4 (a content change) — the golden-set re-run is owed (found by the TQ-4a grounding)

P3-TQ-4a adds one axis-2 rubric item (`axis2/frontend-feedback`, the no-animation-gating rule from TQ-F7) → `rubric-software` v4. The v3 rates (TPR 1.000, TNR 0.500, measured 2026-09-17 on Opus 5) were measured against v3 CONTENT, so they no longer characterize this bundle; P-T06-5 gates judge-MODEL changes, so this is not a STOP — the bundle carries the rates forward with an explicit "re-run owed" sentence. **Ask:** approve one more golden-set rider on the subscription lane (26 cases, ≈$2.40 API-equivalent, $0 cash, the same pre-registration shape) after TQ-4a lands, so v4's rates are its own. Recommendation: yes, batched with the next rider anyway.

### B14. The verification walk needs a browser inside the sandbox — an adoption decision (TQ-4b; found by the TQ-4a grounding)

A16's DOM-asserting walk of the acceptance lines needs a headless browser INSIDE the verification sandbox's empty network namespace beside the deliverable's dev server. Nothing in `components.lock` supplies one (S16: a new component = a lock entry with all fields + your ratification). TQ-4a ships the walk's schema and records the e2e rung honestly as "no browser is adopted for verification walks yet". Host facts, probed: Google Chrome 153 is installed under `/opt`, which no sandbox class binds read-only, so it is unreachable in every sandbox today; a `chrome-headless-shell` sits in a user cache (not installed by Sinet, unpinned); Playwright is not installed; node/npm are at `/usr/bin` and therefore already inside every class (which is why the evidence rungs need no adoption).

Two candidates, costed in the brief §TQ-4b: **(1) a pinned `chrome-headless-shell` bind-mounted read-only into the sandbox and driven by platform code over the Chrome DevTools Protocol** (smallest trust base; Sinet writes the client; a new read-only bind = a structural sandbox profile edit) or **(2) Playwright as an adopted npm component** (no client to own; but a Node runtime and `node_modules` inside the boundary, browser downloads as a second pinned artifact, and it brings a test runner Sinet does not want). Two facts no browser choice settles, both for your explicit ruling: (a) nothing in the sandbox code brings loopback up inside the empty netns — a dev server binding 127.0.0.1 in there is an ASSUMPTION, so TQ-4b must probe it first; (b) A16 names class C1 (read-only workspace) while the landed check runner uses C2 (writable, network-off) — a Vite dev server writes a cache, so either the walk runs at C2 or C1 gains writable scratch.

**Ask:** (i) which candidate to adopt (recommendation: (1), pinned headless shell + platform CDP client, after the loopback probe); (ii) the walk's confinement class (recommendation: C2, network-off is what P-T06-2 requires anyway). Both land as an S16 adoption at a gate plus a sandbox profile edit under the host safety gates.
