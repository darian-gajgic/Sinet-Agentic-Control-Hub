...
| utility | **0/10 (0%)** | HARNESS DEFECT (F2b): the runner sent NO json_schema for output-contract suites while the utility contract demanded strict JSON fields → the free-text draft carried no `{what,wrong,recommend}`. Fixed in drain B6 + re-run (below) |

embedder = post-gate (no suite). None was calibrated (correct — these carry no label margin); the numbers above are the honest output-contract observations, not suppressed.
