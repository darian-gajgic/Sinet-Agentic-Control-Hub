> **EXPIRED 2026-09-18 — merged into main as `b82a0ed` (grounding `5cb5b43`, impl `8fdcdc5`, drain r1 `451cc7f`, inline nit `f668cc8`; evaluation FAIL on test gaps → drained, re-check PASS; landing battery 48 pkgs `-p 1` 0 FAIL 0 SKIP; CONVENTIONS §77). Single-use artifact: never read as truth; code + spec are the truth. Superseded at drain: the disagreement sentence is path-neutral ("Verification found step S's promise unmet… Verification and the review disagree, so a person has to look."). §9 OQ-1 is packet P3-TQ-7; OQ-2/OQ-3 are on the ledger.**
