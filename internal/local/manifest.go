...
func Manifest() []SeatRecord {
	...
	return []SeatRecord{
		...
		},
		{
			...
			Note: "RE-PULLED + HASHED at B4-7 (R19): the B4-5 rate-limit had cleared; resumed to completion (6.98 GB, sha256 above). Ran the B4-7 bakeoff (25.0% intake-triage, slower than the 9B — did NOT flip the default). The alias flip stays operator-ratification-blocked pending the card-vs-spec license conflict below.",
		},
		{
			...
			Note: "S12.3 names Guardian 8B; the current card is Granite Guardian 3.3-8B (3.3 vs 4.1 comparison = S12.9 R24 — but the 4.1 GGUF does not exist at the guessed repo, honest blocker; 3.3-only or two-way). Quant policy: Q5/Q6 for ≤9B on pool12; KV cache stays fp16/q8_0 (never quantized). RE-PULLED + HASHED at B4-7 (R19): resumed to completion (4.20 GB, sha256 above). This is the entailment DEFAULT seat, consumed by B4-7's entailment measurement (S12.9).",
		},
		{
		...
		},
		{
			...
			License: License{SPDX: "Apache-2.0", VerifyDate: vd},
			Files:   []GGUFFile{{Repo: "mradermacher/Arctic-Text2SQL-R1-7B-GGUF", Name: "Arctic-Text2SQL-R1-7B.Q5_K_M.gguf", SHA256: "157cc3e1caafb02a7e4c7abc6e23fbe1bb1b75ef1623fc54885bd17b1a8c0c5d"}},
			Quant:   "Q5_K_M", Pool: "pool12", ContextLen: 32768, ServingContext: 8192, GPUSeated: true, Servable: true, Pulled: true,
			Note: "Enabled at v0; its query-surface consumer (the S12.3 guardrail stack: read-only conn, allowlisted views, single-statement parse, LIMIT+timeout, audit-logged, flagged lower-confidence) is S14/B5 — not built here. RE-PULLED + HASHED at B4-7 (R19): resumed to completion (5.44 GB, sha256 above). Its consumer is S14/B5, not the v0 default path or the smoke.",
		},
		{
