...
var v0Types = []TypeSpec{
	...
	minted("chat.turn_abandoned", FamilyPlatform, VerdictAdmit,
		"internal/chat/chat.go (EventTurnAbandoned); emitted internal/chat/turns.go AbandonTurn and ReconcileOrphanedTurns",
		"the S15.7 hard-stop, which is an ACT and therefore has a record (OQ6). Payload {session, turn, kind, reason}. The turn row survives as abandoned and a result that completes anyway is still recorded honestly — stopping a turn ends the waiting, it does not unmake what happened. A handed-off TASK is never cancelled by this verb: chat claims no lifecycle authority over a task (4.5's cancel is the task's own verb). A SECOND emitter is bootstrap reconciliation: a turn is running only while its driving request is in flight in THIS process, so a running row at startup was orphaned by a crash and nothing will ever settle it — reason distinguishes the two (hard_stop vs process_restart), because a person's act and the platform closing a window nobody closed are different facts"),
