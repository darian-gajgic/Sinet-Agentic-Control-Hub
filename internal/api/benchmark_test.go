...
func TestPartCCountersArePinned(t *testing.T) {
	...
	if v != 23 {
		t.Errorf("user_version = %d, want 23 (0001–0017 untouched, 0018 is this packet's, 0019 is B6-3A's, 0020 is B6-7's, 0021 is B6-9's, 0022 is P3-RW-3's, 0023 is P3-RW-7's)", v)
	}
	...
	// re-create) and P3-RW-7's 0023 (the onboarding arm on the same edge).
	sqls, err := filepath.Glob("../storage/migrations/*.sql")
	if err != nil {
		t.Fatal(err)
	}
	for _, p := range sqls {
		if filepath.Base(p) > "0023_zzz" {
			t.Errorf("unexpected migration %q — part C adds exactly 0018, B6-3A exactly 0019, B6-7 exactly 0020, P3-RW-3 exactly 0022 and P3-RW-7 exactly 0023", filepath.Base(p))
		}
	}
}
