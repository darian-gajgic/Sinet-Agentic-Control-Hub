...
func TestRouteSelectsActiveWorkerDeterministically(t *testing.T) {
	...
	// constant's home).
	if d1.Model != "claude-opus-5" || d1.Lane != "anthropic" || d1.WindowTokens != worker.DefaultWindowTokens {
...
func TestDefaultDutyMapShape(t *testing.T) {
	...
	// Seat mix ratified by the operator 2026-09-17 (P3/gates/rework-sitting-gate.md
	// item B7 + §Answers), which supersedes the B3 gate's advisor split: one
	// model on every seat the map carries. Execution's row is the seat of LAST
	// RESORT — the configured lane order prefers the Kimi lanes ahead of it.
	want := map[string]string{
		worker.DutyExecution: "claude-opus-5",
		worker.DutyPlanning:  "claude-opus-5",
		worker.DutyJudge:     "claude-opus-5",
	}
