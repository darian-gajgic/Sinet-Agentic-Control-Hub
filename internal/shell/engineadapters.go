...
func engineAdapters(deps engineAdapterDeps) map[string]adapters.Adapter {
	...
	if err != nil {
		// A lane document that will not load is not a reason to run without
		// lanes: it is a reason to say so. The adapter still registers, so a
		// dispatch onto an uncommissioned lane still refuses by name rather
		// than falling through to some other substrate.
		deps.Logger.Warn("opencode: seed lane documents did not load; no lane can be commissioned until they do", "err", err)
	}
...
// nobody. This is the one call the key ceremony makes, kept beside the
